...
// Options are as follows: Andelsbolig, Ejerlejlighed, Fritidsgrund, Helårsgrund, Hus, Landejendom, Rækkehus, Sommerhus, Villalejlighed til salg
// It should however be noted that as of now, only Andelsbolig functions properly due to each category seemingly having different informations on their "house" pages,
// and thus would require a overhaul of the scraping part to make up for the different types
// sample command for running the script:
// node estateScraper.js "Andelsbolig"
...
(async () => {
  ...
  try {
    ...
    const linkSelector =
      ".filters-panel__seo-tags-container .filters-panel__seo-tag";
    ...
    for (let h = 1; h < maxPages; h++) {
      let houseTilesSelector =
        ".list__item .tile:has(.tile__top-wrap .tile__logo)";
      ...
      while (i < houseTiles.length) {
        ...
        await Promise.all([
          ...
          page.waitForSelector(
            ".case-info__property__info__main__title__address"
          ),
        ]);
        ...
        let refreshedHouseTiles = await page.$$(
          ".list__item .tile:has(.tile__top-wrap .tile__logo)"
        );
